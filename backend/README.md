...
Collection sync also reads the public Steam DLC catalog for Steam-linked base games. New DLCs are nested under the parent as **Not Started**, even when IGDB has no DLC listing; existing DLC states and manually removed catalog entries are respected. Steam's public catalog lists available DLCs, **not proof of account ownership**. Imported rows are marked `Steam catalog` for that reason. Catalog responses are cached for 30 days, and a bounded number of uncached games are scanned per sync; remaining games continue on later syncs.
