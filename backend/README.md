...
In **Videogames → User Settings**, enter a 17-digit SteamID64 or numeric `https://steamcommunity.com/profiles/...` URL and enable automatic imports. Vanity URLs are not accepted; use the numeric profile ID. The Steam profile and game details must be public. Wishlist sync needs no key. Owned-library sync uses Steam's supported `IPlayerService/GetOwnedGames/v1` endpoint and therefore needs a 32-character Steam Web API key. Enter it in User Settings, or set `STEAM_WEB_API_KEY` on the server. A key entered in User Settings is encrypted server-side per user and is exposed to the client only as a configured/not-configured flag. On Linux, the encryption key defaults to `.steam-secrets.key` beside the SQLite database; keep that file in the persistent database volume, or set `STEAM_SECRET_ENCRYPTION_KEY`. Steam passwords are never requested or stored.
...
Administrators manage the shared Platform, Source and Type vocabularies and their Platform × Source and Source × Type compatibility rules. Copy forms use these rules instead of fixed client-side assumptions. Historical copies use the same Platform vocabulary and are stored separately with played hours; they do not represent current ownership, but their time contributes to copy and combined playtime modes. Retryable Steam/IGDB metadata notices are reported separately from sync failures, and collection sync validates the saved API key before starting.
...
Public Steam DLC catalogs sync separately from the account's wishlist and owned-library settings for Steam-linked base games and games whose DLCs were previously linked through the editor. New DLCs are nested under the parent as **Not Owned**, even when IGDB has no DLC listing. Manual DLCs remain in the same list. A close, unambiguous title match links a manual row to the Steam DLC without changing personal progress; distinct and ambiguous titles stay separate. Existing DLC states and manually removed catalog entries are respected. Steam's public catalog lists available DLCs, **not proof of account ownership**. Imported rows are marked `Steam catalog` for that reason. Catalog responses are cached for 7 days, and a bounded number of uncached games are scanned per run; remaining games continue on later runs. The game's DLC editor can refresh the Steam catalog on demand, including for games without a linked Steam copy.
