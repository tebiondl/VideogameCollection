...
Administrators manage the shared Platform, Source and Type vocabularies and their Platform × Source and Source × Type compatibility rules. Copy forms use these rules instead of fixed client-side assumptions. Historical copies are stored separately with a console and played hours; they do not represent current ownership, but their time contributes to copy and combined playtime modes. Retryable Steam/IGDB metadata notices are reported separately from sync failures, and collection sync validates the saved API key before starting.
