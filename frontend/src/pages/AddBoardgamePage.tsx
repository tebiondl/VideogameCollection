...
export function AddBoardgamePage() {
  ...
  useEffect(() => {
    ...
    const pollSession = async () => {
      ...
      try {
        const res = await fetchWithAuth('/boardgames/smart-import/latest');
        if (res.ok) {
      ...
      for (const file of selected) {
        ...
        if (type === 'excel') {
          try {
            ...
            fd.append('file', file);
            const res = await fetchWithAuth('/boardgames/smart-import/excel-sheets', {
              method: 'POST',
              body: fd,
            });
            if (res.ok) {
  ...
  const startSmartImport = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetchWithAuth('/boardgames/smart-import/', {
        method: 'POST',
        body: formData,
      });
  ...
  const updateSmartItemStatus = async (item: any, newStatus: string) => {
    try {
      await fetchWithAuth(`/boardgames/smart-import/items/${item.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  const setAllItemsStatus = async (status: 'accepted' | 'rejected') => {
    ...
    try {
      const res = await fetchWithAuth(`/boardgames/smart-import/sessions/${smartSession.id}/bulk-status`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  const commitSmartImport = async () => {
    ...
    try {
      const res = await fetchWithAuth(`/boardgames/smart-import/commit/${smartSession.id}`, { method: 'POST' });
      if (res.ok) {
        navigate('/dashboard/boardgames');
      } else {
  ...
  const checkSimilar = async (targetName: string) => {
    const checkRes = await fetchWithAuth('/boardgames/check-similar', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  const saveSmartEdit = async () => {
    try {
      await fetchWithAuth(`/boardgames/smart-import/items/${editingItem.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          ...editingItem,
  ...
  return (
    <div className="container" style={{ padding: '2rem 1.5rem', maxWidth: '800px' }}>
      <div style={{ marginBottom: '2rem' }}>
        <Link to="/dashboard/boardgames" className="btn btn-ghost" style={{ padding: '0.5rem 0' }}>
          <ArrowLeft size={18} />
          Back to Videogames
        </Link>
      </div>
