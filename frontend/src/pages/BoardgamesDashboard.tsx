...
export function BoardgamesDashboard() {
  ...
  const openNewGame = (section: LibrarySection) => { setEditingGameId(null); setGameDraft(newGameDraft(section)); setExpansionName(''); setBggQuery(''); setBggResults([]); setBggFeedback(null); };
  const openEditGame = (game: Boardgame) => { setEditingGameId(game.id); setGameDraft({ ...game }); setExpansionName(''); setBggQuery(game.name); setBggResults([]); setBggFeedback(null); };
