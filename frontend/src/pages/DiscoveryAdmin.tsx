  ...
  const content = <>
    ...
    {!settings ? <p className="disc-empty">{error ? 'Settings could not be loaded. Reload to try again.' : 'Loading settings…'}</p> : <div className="disc-admin-grid">
      <section className="disc-panel"><div className="disc-section-heading"><h2>Steam library & wishlist</h2><span className="disc-badge">{settings.steam_id ? settings.sync_enabled ? 'AUTO SYNC ON' : 'CONNECTED · PAUSED' : 'NOT CONNECTED'}</span></div>
        ...
        <div className="disc-sync-status"><dl><div><dt>Last sync</dt><dd>{timestamp(settings.last_sync_at)}</dd></div><div><dt>Next automatic sync</dt><dd>{settings.sync_enabled ? timestamp(settings.next_sync_at) : 'Paused'}</dd></div><div><dt>Wishlist import</dt><dd>{settings.last_import_count} new games</dd></div><div><dt>Library import</dt><dd>{settings.last_owned_import_count} new collection games</dd></div><div><dt>IGDB auto-completion</dt><dd>{settings.last_igdb_match_count} games matched</dd></div></dl>{syncing && <p role="status">Sync in progress… This page updates automatically.</p>}{settings.sync_error && <p className="disc-alert error" role="alert">{settings.sync_error}</p>}{settings.sync_warning && <p className="disc-alert" role="status">{settings.sync_warning}</p>}</div>
