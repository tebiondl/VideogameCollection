  ...
  return <div className="old-copies-editor">
    {copies.map((copy, index) => {
      ...
      return <div className="old-copy-row" key={copy.id || index}>
        <label>Console<select className="form-input" value={copy.console} onChange={event => update(index, { console: event.target.value })}>{consoles.map(console => <option key={console}>{console}</option>)}</select></label>
